...
原则：新增敌人必须带来新的应对方式，不通过单纯叠血、叠伤害或无预警加速制造难度。

### 迅猛兽（Razor Hound）

- [x] 独立透明贴图 `assets/sprites/enemy_razor_hound.png`
- [x] 新增独立 EnemyType 与高速移动 Intent
- [x] 每次沿可见路径最多移动 2 格，允许转弯但不能穿墙、踩火、踩刺或跳坑
- [x] 基础 18 HP、关卡加成封顶 10；贴身后下一次 Intent 才攻击，禁止移动后同拍咬人
- [x] 第 5 关开始出现；前期最多 1 只，第 15 关起最多 2 只
- [x] Observation 输出两格路径；Candidate 标出冲刺终点与下一拍咬击威胁
- [x] Renderer 增加高速残影与路径预警
- [x] 固定场景测试：绕墙、避火、玩家占据预告路径时停车、不给出生杀

### 整十关 Boss

第 10、20、30、40、50、60 关使用专用 Boss 房，不和普通随机关卡只做数值叠加。每个 Boss 至少包含：
...
第一只 Boss：**棱镜守卫（Prism Warden）**。

- [x] 独立透明贴图 `assets/sprites/boss_prism_warden.png`
- [x] 护盾阶段：普通攻击不能直接削血
- [x] 激光提前完整预警 1 回合，出生保护期内不得攻击
- [x] 玩家诱导激光击中固定反射柱，使光束反射回 Boss 核心
- [x] 完成 3 次不同镜柱反射后破盾，核心暴露 3 回合（经实机可击杀性调整）
- [x] 血量三分之二进入第二阶段：有预警的邻格横扫、掩体击碎、开放窗口换位；规则代理和混合决策可无伤通关
- [ ] 增加扇形与旋转扫射组合，但每次至少保留一条经 Solver 验证的安全路线
- [x] Dash 可穿过弹道触发无敌帧，但不能成为唯一解
- [ ] Boss 失败后快速重试，Replay 保存阶段、预警格、反射次数和破盾窗口
- [x] 固定测试：无出生杀、无必吃伤害、反射可稳定命中、Boss 可在有限回合内击败

第 10～100 关共 10 只 Boss 的设定、透明贴图和专属特效已准备完成，详细规格见
`JEV_ARENA_BOSS_ROADMAP.md`。战斗逻辑仍按关卡顺序逐只实现和验证，避免一次堆入十套未经测试的机制。

第 20 关熔炉三头机已加入半血后二阶段：三列熔岩波、短暂燃烧格和开放窗口换位；火球与熔岩波独立预警、三座冷却阀破盾、大锻炉房间、熔岩边道、弓箭与血包补给均已接入，规则代理及默认 NanoJev 混合策略通过固定场景无伤通关验证。第 30 关风暴合唱环现有 132 点生命、错位接地柱和连锁闪电；降至三分之二血量后导电位移到两翼，雷爆伤害提高但范围不变，默认混合策略通过固定场景无伤通关。第 40 关时序螳螂现有横切、锁定近斩、预告跃迁、时间锚破盾与二阶段锚点后移。第 50 关虚空钓手已接入暗色异形房、引力雷/锁定光束、三节点吸离装甲与破盾输出窗口。第 60 关钢铁园丁已接入两轮生长的荆棘墙、焚烧线/荆棘爆发、四根盘热回流破盾。第 70 关镜像炽天使已接入延迟复制、镜像方向预警、三镜锁破盾与局部沉默。第 80 关攻城利维坦已接入双回合轨道炮、可移动/重建掩体、四锁破盾。第 90 关归零织机已接入四类动作轮换封锁、两回合预警的临时断裂地板和四个编号节点破盾。第 100 关顶点裁决者已接入四种基础招式、熔锁雷幕与镜冲引力两组同屏合招、终审判词的上诉反制；规则代理和默认混合策略均通过固定场景无伤通关。第 10～100 关 Boss 房轮廓与补给组合不重复；终章独立弹体、完整位移动画和专属结算尚未实装。

第 30 关已改为连锁雷前慢速侧移、雷爆前快速压近并改变高度，锁定后保持预警路径。第 40 关已补按玩家站位换侧、横切兼纵向追近、锁定玩家原位置的跳杀、二阶段时间锚后移与核心开放时换位；玩家若留在跳杀目标格将受伤，站在时间锚上接招则可破盾，弹药耗尽可近战补刀。下一步是绕掩体的深度追击、更多佯攻与残影组合，具体制作标准见 `JEV_ARENA_BOSS_ROADMAP.md`。

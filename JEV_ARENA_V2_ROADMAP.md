...
第 10、20、30 关使用专用 Boss 房，不和普通随机关卡只做数值叠加。每个 Boss 至少包含：
...
第 20 关熔炉三头机已加入半血后二阶段：三列熔岩波、短暂燃烧格和开放窗口换位；火球与熔岩波独立预警、三座冷却阀破盾、大锻炉房间、熔岩边道、弓箭与血包补给均已接入，规则代理及默认 NanoJev 混合策略通过固定场景无伤通关验证。第 30 关风暴合唱环现有 132 点生命、错位接地柱和连锁闪电；降至三分之二血量后导电位移到两翼，雷爆伤害提高但范围不变，默认混合策略通过固定场景无伤通关。第 40 关时序螳螂现有横切、锁定近斩、预告跃迁、时间锚破盾与二阶段锚点后移，规则代理和默认混合策略通过固定场景无伤通关；第 50～100 关尚未实装。

...
[![Tests](https://img.shields.io/badge/Tests-164%20passing-22C55E)](#测试与验证)
...
| 五类敌人 | 追击怪、冲锋怪、炸弹怪、射手怪与迅猛兽各有不同的移动和攻击方式；迅猛兽会预告两格绕障冲刺，下一拍才咬人 |
...
| 第二只 Boss | 第 20 关熔炉三头机：105 点生命，半血后二阶段熔岩波扩至三列、火球短暂点燃地面；会召唤一次机械爆裂幼体；冷却阀仍可化解对应主火线 |
| 第三只 Boss | 第 30 关风暴合唱环：132 点生命，连锁雷前向左侧换位、雷爆前向右侧压近；降至三分之二血量后导电位移到两翼，四柱接地回灌破盾 |
| 第四只 Boss | 第 40 关时序螳螂：横切追击并锁定跳杀；时间锚反制后留近战窗口，再撤到远侧释放有预警的三格残影波 |
| 第五只 Boss | 第 50 关虚空钓手：引力雷时逼近节点，远程光束前跃迁到远侧；在 3 处不同引力节点接雷吸离装甲，再攻击核心 |
| 第六只 Boss | 第 60 关钢铁园丁：种子两轮长成荆棘墙，焚烧线与荆棘爆发交替；会召唤一次藤蔓猎兽；引火烧掉 4 条藤蔓并让热量回流破盾 |
...
默认 NanoJev 混合策略会主动站上时间锚：锚点能挡近斩，守住后可接跳杀破盾，随后立刻近战或射击 Boss；螳螂先留在近处供反击，之后撤离并预告三格残影波。跳杀和残影波锁定后都不会临时追踪改点。

第 50 关虚空钓手实机画面（紫色十字为引力雷范围，青色圆环提示可反制节点）：

![第 50 关虚空钓手与引力雷预警](assets/screenshots/boss_void_angler.png)

虚空钓手 108 点生命，交替使用引力雷和 22 伤害锁定光束。布雷时向节点逼近，准备光束时先跃迁到远侧再锁定目标；玩家在 3 处不同节点接雷后装甲脱落，核心开放 4 回合。牵引不会直接把玩家拖入坑。Boss 房提供手枪、能量弹匣和血包，默认混合策略与规则策略均通过无伤通关测试。

第 60 关钢铁园丁实机画面（绿色根盘可引导焚烧线，藤蔓长成后会挡路）：

![第 60 关钢铁园丁与焚烧线预警](assets/screenshots/boss_iron_gardener.png)

![第 60 关园丁召唤藤蔓猎兽](assets/screenshots/boss_iron_summon.png)

钢铁园丁 108 点生命，焚烧线伤害 22，锁定荆棘伤害 18。种子两轮长成可阻路的荆棘墙；园丁可召出一只独立贴图的藤蔓猎兽。玩家引火烧掉 4 条不同藤蔓并站在对应根盘，让热量回流破盾，核心开放 4 回合。Boss 房提供手枪、弹药和血包，默认混合策略与规则策略均无伤通关。

第 70 关镜像炽天使实机画面（粉色轨迹是下一轮真实射线；紫色十字是锁定旧位置的镜片爆裂范围）：

![第 70 关镜像炽天使与镜锁预警](assets/screenshots/boss_mirror_seraph.png)

镜像炽天使有 168 点生命和三处镜锁，会延迟一个敌方回合复制最近一次非等待动作；左右方向互换，冲刺复制射线伤害 26，普通复制射线伤害 18。复制方向动作时，还会在玩家旧位置留下紫色镜片十字预警，下一轮覆盖中心与四邻格并造成 14 伤害；即使射线击碎镜锁也会爆裂，必须主动走位。左、右、下镜锁直接标字，右侧常驻提示「右→左锁、左→右锁、下→下锁」和当前复制方向。玩家引导射线击碎三处不同镜锁后可输出 4 回合，需要完成至少两轮破盾。复制治疗不会额外消耗药包；复制 EMP 只造成短暂局部沉默。Boss 房有 3 个血包和 2 个能量弹匣。

第 80 关攻城利维坦实机画面（橙色为两轮蓄力后贯穿的轨道炮，青色为掩体遮挡的安全段）：

![第 80 关攻城利维坦轨道炮预警](assets/screenshots/boss_siege_leviathan.png)

利维坦有 144 点生命，炮击伤害 36；三块掩体可推动，也会在炮击后移动。掩体挡下一炮后碎裂，两个敌方回合后在固定位置重建；垂直炮线被掩体截断时，可把爆炸折向对应装甲锁。四锁击碎后核心开放 6 回合，通常需要两轮破盾。该关采用独立的阶梯形攻城房，提供 3 个血包、1 个能量弹匣、弓和 2 组箭束。模型候选动作现能正确处理推动掩体，不会把掩体误当成怪物导致推理中断。
...
关卡会逐步增加敌人、障碍和危险地形；常规敌人生命每关 +1，迅猛兽生命加成封顶 10，各类攻击伤害分段增长，同时分阶段强化不同怪物：

| 关卡 | 节奏变化 |
| --- | --- |
| 第 5／15 关 | 迅猛兽开始出现／最多增至两只；每次沿预告路线最多跑两格 |
| 第 6 关 | 追击怪移动加快 |
| 第 12 关 | 冲锋怪移动加快，蓄力时间缩短 |
| 第 18 关 | 射手怪移动加快，瞄准时间缩短 |
| 第 24 关 | 炸弹怪小幅加快，冲锋怪进入第二速度档 |

越过首次提速关卡后，每 12 关再缩短一档行动间隔，最低仍保留 1 轮预警。迅猛兽单次位移最长，普通追击怪行动最频繁，冲锋怪次之，射手怪和炸弹怪更慢；黄色冲锋轨迹、青色迅猛兽路径与粉色射击准线有不同的预警样式。
...
```powershell
.\.venv\Scripts\python.exe scripts\play_gui.py --agent nanojev
```

默认每次启动使用新随机种子；需要复现关卡布局时可追加 `--seed 61005`。第 10 关按 R 重试也会换一组镜子位置。
...
本地模型输入超过长度上限时会由规则策略接管该步；第 80 关的混合模式使用明确标注的规则辅助，避免模型在掩体间循环。
...
当前环境、战斗、地图、武器、存档、Replay、搜索与模型适配共有 **164 项回归测试**。

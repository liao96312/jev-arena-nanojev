...
[![Tests](https://img.shields.io/badge/Tests-135%20passing-22C55E)](#测试与验证)
...
利维坦有 144 点生命，炮击伤害 36；三块掩体可推动，也会在炮击后移动。掩体挡下一炮后碎裂，两个敌方回合后在固定位置重建；垂直炮线被掩体截断时，可把爆炸折向对应装甲锁。四锁击碎后核心开放 6 回合，通常需要两轮破盾。该关采用独立的阶梯形攻城房，提供 3 个血包、1 个能量弹匣、弓和 2 组箭束。第 10～80 关已实装 Boss；第 90～100 关仍是[设计规划](JEV_ARENA_BOSS_ROADMAP.md)。

第 10～80 关 Boss 房的外轮廓与补给组合各不相同；关键道具均经可达性检查，默认混合策略和规则策略均通过无伤通关测试。

...
[![Tests](https://img.shields.io/badge/Tests-130%20passing-22C55E)](#测试与验证)
...
镜像炽天使有 168 点生命和三处镜锁，会延迟一个敌方回合复制最近一次非等待动作；左右方向互换，冲刺复制射线伤害 26，普通复制射线伤害 18。玩家引导射线击碎三处不同镜锁后可输出 4 回合，需要完成至少两轮破盾。复制治疗不会额外消耗药包；复制 EMP 只造成短暂局部沉默。Boss 房有手枪、能量弹匣和血包，默认混合策略与规则策略通过无伤通关测试。第 10～70 关已实装 Boss；第 80～100 关仍是[设计规划](JEV_ARENA_BOSS_ROADMAP.md)。

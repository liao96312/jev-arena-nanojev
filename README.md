...
[![Tests](https://img.shields.io/badge/Tests-158%20passing-22C55E)](#测试与验证)
...
镜像炽天使有 168 点生命和三处镜锁，会延迟一个敌方回合复制最近一次非等待动作；左右方向互换，冲刺复制射线伤害 26，普通复制射线伤害 18。左、右、下镜锁现直接标字，右侧常驻提示「右→左锁、左→右锁、下→下锁」和当前复制方向。玩家引导射线击碎三处不同镜锁后可输出 4 回合，需要完成至少两轮破盾。复制治疗不会额外消耗药包；复制 EMP 只造成短暂局部沉默。Boss 房有 3 个血包和 2 个能量弹匣，默认混合策略与规则策略通过无伤通关测试。
...
当前环境、战斗、地图、武器、存档、Replay、搜索与模型适配共有 **158 项回归测试**。

...
[![Tests](https://img.shields.io/badge/Tests-138%20passing-22C55E)](#测试与验证)
...
归零织机有 168 点生命，每轮只封锁普通移动、近战、远程或技能中的一类，仍保留另一种移动手段。断裂地板提前两轮标记，坠入伤害 20，下一轮恢复。四节点反向写入使核心开放 6 回合；错误顺序只重置节点进度。独立房间提供 2 个血包、3 个能量弹匣、弓和 1 组箭束。第 10～90 关已实装 Boss；第 100 关仍在[设计规划](JEV_ARENA_BOSS_ROADMAP.md)。

第 10～90 关 Boss 房的外轮廓与补给组合各不相同；关键道具均经可达性检查，默认混合策略和规则策略均通过无伤通关测试。
